...
ames_test$pred_price = exp(predict(model.AIC, ames_test))

mean(ames_test$pred_price)
...
Answer: 2. When predicting to `ames_train`, the RMSE for `model.AIC` is higher than the RMSE for `model.full`.  However, when predicting to `ames_test`, the RMSE for `model.full` is higher.

```{r Q2}
# type your code for Question 2 here, and Knit

predict_test.AIC = exp(predict(model.AIC, ames_test))
resid.AIC.test = ames_test$price - predict_test.AIC
rmse.AIC.test = sqrt(mean(resid.AIC.test^2))

predict_train.AIC = exp(predict(model.AIC, ames_train))
resid.AIC.train = ames_train$price - predict_train.AIC
rmse.AIC.train = sqrt(mean(resid.AIC.train^2))

predict_train.full = exp(predict(model.full, ames_train))
resid.full.train = ames_train$price - predict_train.full
rmse.full.train = sqrt(mean(resid.full.train^2))

cat("AIC, test:", rmse.AIC.test, "\n")
cat("AIC, train:", rmse.AIC.train, "\n")
cat("Full, test:", rmse.full, "\n")
cat("Full, train:", rmse.full.train, "\n")
```
...
Answer: False. In general terms, model is optimized for training data, as shown in the previous example.
```{r Q3}
...
n = nrow(ames_train)
k_BIC = log(n)
model.BIC = stepAIC(model.full, k = k_BIC, trace=0)
...
# Get dataset of predictions and confidence intervals
# By default, confidence interval=0.95
...
pred.test.MPM.outliers = 1 - pred.test.MPM.coverage
pred.test.MPM.outliers
```
...
Answer: False. It has a tendency to under-estimate house prices.
