...
Answer: 075: 1 story planned unit development

```{r Q7}
# type your code for Question 7 here, and Knit
lm2 = lm( log(price) ~ Overall.Cond + Overall.Qual, data=ames_train)

ames_predict = ames_train
ames_predict$price_prediction = predict(lm2)

ames_predict %>% 
  group_by(MS.SubClass) %>%
  summarise(median = median(price_prediction)) %>%
  arrange(., desc(median))
...
#Let's find the index of the max value
which.max(hatvalues(lm2))
...
Answer: Abnorm and Partial. Both groups have influential points from the normal regression line.
...
ames_train_subset = subset(ames_train, Sale.Condition == 'Normal')

normal = count(ames_train_subset)
total = count(ames_train)
...
Answer: True, restricting the model to only include observations with normal sale conditions increases the $R^2$ from 0.547 to 0.575.

```{r Q12}
# type your code for Question 12 here, and Knit
lm3 <- lm(log(price) ~ log(area), data=ames_train_subset)
summary(lm3)
